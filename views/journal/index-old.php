...
<div class="order-index">
    ...
    <div class="row">
        <div class="col-lg-2 highlights-filter">
            <?= $form->field($searchModel, 'highlightLine')->dropDownList(Order::highlightsColors(), ['prompt' => '--Выбрать--']) ?>
        </div>
        <div class="col-lg-2 draw-items">
            <?= $form->field($searchModel, 'changeStatusForm')->dropDownList(Order::statusNamesArray(), ['prompt' => '--Выбрать--']) ?>
        </div>
        <div class="col-lg-2">
            <?= $form->field($searchModel, 'changeColorForm')->dropDownList(Order::colorsArray(), ['prompt' => '--Выбрать--']) ?>
        </div>
        <div class="col-lg-3">
    ...
    <?php echo GridView::widget([
        ...
        'rowOptions' => function ($model, $key, $index, $grid) {
            return ['style' => $model->getSearchStatusStyle(), 'class' => 'row-item'];
        },
        'columns' => [
    ...
    <div class="panel-default">
        <div class="panel-body">
            <div class="row">
                <?php foreach($searchModel->outfits as $key => $value) : ?>
                    <div class="col-md-2">
                        <input type="radio" id="<?= $value['button_id'] ?>" class="outfit" data-type="<?= $key ?>" value="0">
                        <label for="<?= $value['button_id'] ?>"> <?= $value['button_name'] ?></label>
                    </div>
                <?php endforeach; ?>
                <div class="col-md-4">
                ...
                <div class="col-md-2">
                    <?= Html::a('Печать', '#', ['class' => 'btn btn-primary pull-right outfit-btn', 'id' => 'outfit-btn']) ?>
                </div>
            </div>
        </div>
    </div>
