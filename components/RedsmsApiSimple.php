...
class RedsmsApiSimple
{
    ...
    const VALIDITY_PERIOD_MAX = 86400;
    ...
    //public function sendSMS($to, $text, $from, $route = RedsmsApiSimple::VOICE_TYPE)
    {
        $methodUrl = 'message';
    ...
    public function sendVoice($to, $text, $from, $route = RedsmsApiSimple::VOICE_TYPE)
    {
        $methodUrl = 'message';
        $to = is_array($to) ? $to : [$to];
    ...
    protected function getCurlResult($curlResource)
    {
        ...
        file_put_contents('sms-logs.txt', date('d.m.Y H:i:s').' response - '.print_r($response, true)."\n", FILE_APPEND);
        file_put_contents('sms-logs.txt', date('d.m.Y H:i:s').' info - '.print_r($info, true)."\n", FILE_APPEND);

        if (json_last_error() != JSON_ERROR_NONE) {
            file_put_contents('sms-logs.txt', date('d.m.Y H:i:s').' - '.print_r('Error response format', true)."\n", FILE_APPEND);
